...
def main():
    ...
    param_grid = {'num_boost_round': [100, 250, 500],
                  'eta': [0.05, 0.1, 0.3],
                  'max_depth': [6, 9, 12],
                  'subsample': [0.9, 1.0],
                  'colsample_bytree': [0.9, 1.0]}
    kf = KFold(n_splits=5, shuffle=True, random_state=123)
    gs = GridSearchCV(XGBClassifier(eval_metric='auc', nthread=1, num_class=1),
                      param_grid, cv=kf)
    ...
    for t in range(100):
        ...
        csvFile = open("XGBy110D_proba.csv", 'a', newline='')
        writer = csv.writer(csvFile, dialect='excel')
        writer.writerow(probass_y)
        csvFile.close()

        csvFile = open("XGBtest110D_index.csv", 'a', newline='')
        writer = csv.writer(csvFile, dialect='excel')
