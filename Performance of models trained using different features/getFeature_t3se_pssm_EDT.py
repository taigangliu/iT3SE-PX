...
def getMatrix(dirname,lenList):
    ...
    m = len(pssmList)
    reMatrix = np.zeros((m, 4000))
    for i in range(m):
        matrix = divideMartix(dirname + "/" + pssmList[i], lenList[i])
            ...
            for j in range(20):
                for k in range(20):
                    ...
                    for z in range(lenList[i] - lag):
                        sum += (pow((matrix[z, j] - matrix[z + lag, k]), 2))
                    ls.append(sum / (lenList[i] - lag))
        ls = np.array(ls)
        reMatrix[i] = ls
    return reMatrix
...
def main():
    ...
    train_x.to_csv('train_EDT4000.csv')
    ...
    test_x.to_csv('test_EDT4000.csv')
